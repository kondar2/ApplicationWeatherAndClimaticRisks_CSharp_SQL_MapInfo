...
BEGIN 
   ...
   INTO @vich_second_name, @ost_second_name, @second, @Gr2;
     ...
     IF @ost_second_name IS NULL
		   ...
		   UPDATE Cheb2
		   SET Summa = @sum
		   WHERE /*R_sub = @second*/ ost_char = @ost_second_name
		   ...
		   WHERE /*R_sub = @first*/ vich_char = @vich_first_name OR ost_char = @ost_first_name
   ...
   /*
	  IF @second = (SELECT TOP 1(R_sub) FROM Cheb2 ORDER BY R_sub DESC)
		   Begin
		   UPDATE Cheb2
		   SET Summa = @sum
		   WHERE R_sub = @second
...
BEGIN 
   ...
   /**/
   IF /*@Summ_razn1 IS NOT NULL AND */ @Summa IS NOT NULL
	   BEGIN
	   label1:
	   FETCH NEXT FROM cheb_curs2 INTO @vich_first_name, @ost_first_name, @first, @Gr1, @Summ_razn1, @Summa
              ...
              IF @ost_second_name = @last_el
			   ...
			   FETCH RELATIVE -1 FROM cheb_curs2 INTO @vich_second_name, @ost_second_name, @second, @Gr2, @Summ_razn2, @Summa
				...
				END
				 ...
				 --SET @el_gr = (SELECT (Count(ost_char)) FROM Cheb1 WHERE Gr = @Gr1)
				 ----
				 --IF @ost_first_name = ( Select ost_char From Cheb2 WHERE Gr = 17 order by Gr OFFSET @el_gr ROWS)
